 ...
 * the License.
 */
package ti.bottomsheet.adapter;
...
import ti.bottomsheet.BottomSheet.Style;
import ti.bottomsheet.R;
...
public class DividableGridAdapter extends BaseAdapter {
    ...
    private void visualizeItem(@NonNull final Item item, @NonNull final ItemViewHolder viewHolder) {
        ...
        viewHolder.titleTextView.setTextColor(item.getItemTitleColor());
    ...
    public final void setItemColorAtIndex(final int index,@ColorInt final int color) {

        int viewType = getItemViewType(index);
        AbstractItem itemToSetColor = getItem(index);
        ...
        if (viewType == ITEM_VIEW_TYPE) {
                view = inflateItemView(null);
                ItemViewHolder viewHolder = (ItemViewHolder) view.getTag();
                visualizeItem((Item) itemToSetColor, viewHolder);
